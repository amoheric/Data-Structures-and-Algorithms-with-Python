...
import random  # Import the random module to generate random keys for insertion/deletion
import memory_profiler  # Import the memory_profiler module to measure memory usage
...
class AVLTree:
    # AVL Tree class
    def __init__(self):
        self.root = None  # Initialize the root of the AVL tree
    ...
    def _insert(self, node, key):
        ...
        if not node:
            return Node(key)  # Create a new node if we reach a leaf
        ...
        if key < node.key:
            ...
            # Keys are unique, do not insert duplicates
            return node
